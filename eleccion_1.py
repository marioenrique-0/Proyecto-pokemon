...
def selecion():
    ...
    if "equipo_j1" not in st.session_state: 
        st.session_state.equipo_j1 = {}
    if "equipo_j2" not in st.session_state: 
        st.session_state.equipo_j2 = {}
    ...
    with col_j1:
        ...
        for clave in st.session_state.equipo_j1.keys():
            st.write(clave)
            
    with col_j2:
        st.markdown(f"**Jugador 2 ({len(st.session_state.equipo_j2)}/6)**")
        # Imprimimos las keys directamente
        for clave in st.session_state.equipo_j2.keys():
            st.write(clave)
    ...
    if len(st.session_state.equipo_j1) == 6 and len(st.session_state.equipo_j2) == 6:
        ...
        if st.session_state.jugador_actual == 1:
            equipo_activo = st.session_state.equipo_j1
        else:
            equipo_activo = st.session_state.equipo_j2
...
def mostrar_menu_acciones(num_jugador: int, pokemon_activo: Pokemon, equipo: dict):
    ...
    if st.session_state[var_menu] == "ataques":
        ...
        for nombre_ataque, datos in pokemon_activo.ataques.items():
            if st.button(f" {nombre_ataque}", key=f"btn_atk_j{num_jugador}_{nombre_ataque}"):
    ...
    # --- MENÚ DE CAMBIO DE POKÉMON ---
        ...
        if st.button("⬅ Volver a ataques", key=f"btn_volver_j{num_jugador}"):
            st.session_state[var_menu] = "ataques"
            st.rerun()
...
def pokemones_jugador(jugador: int)-> dict:
    pokemones_j = {}
    if jugador == 1:
        equipo_base = st.session_state.equipo_j1  
    else:
        equipo_base =  st.session_state.equipo_j2
    for pokemon in equipo_base.values():
        pokemones_j[pokemon.nombre] = pokemon 
    return pokemones_j

def atacar(atacante: Pokemon, objetivo: Pokemon, nombre_ataque: str, log: list) -> None:
    if atacante.hp <= 0:
        log.append(("warning", f"{atacante.nombre} está debilitado."))
        return
    ...
    log.append(("write", f"**{atacante.nombre}** intenta usar **{nombre_ataque}**..."))

    if random.randint(1, 100) > precision:
        log.append(("warning", "El ataque ha fallado"))
    else:
        d = daño(atacante, objetivo, potencia, tipo)
        actualizar_hp(objetivo, d)
        log.append(("success", f"¡Es un golpe directo! Causó {int(d)} puntos de daño."))

def mostrar_log(log: list):
    for tipo, mensaje in log:
...
def equipo_tiene_vida(jugador: int) -> bool:
    equipo = pokemones_jugador(jugador)
    for pokemon in equipo.values():
        if pokemon.hp > 0:
            return True
    return False

def iniciar_nuevo_turno():
    st.session_state.accion_j1 = None
    st.session_state.accion_j2 = None
    st.session_state.turno_resuelto = False
    st.session_state.log_turno = []

def mostrar_pantalla_victoria(jugador_ganador: int):
    st.balloons()
    st.title("FELICIDADES")
    st.success(f"🏆 ¡Has ganado, Jugador {jugador_ganador}! 🏆")
    st.write("El equipo rival se quedó sin Pokémon disponibles.")

    if st.button("Volver al Menú Principal"):
        # Reiniciamos todo el estado del combate y de la selección
        for clave in [
            "pantalla_actual", "modo_juego",
            "equipo_j1", "equipo_j2", "jugador_actual",
            "pokemon_activo_j1", "pokemon_activo_j2",
            "accion_j1", "accion_j2",
            "turno_resuelto", "log_turno",
            "viendo_menu_j1", "viendo_menu_j2",
        ]:
...
def mostrar_menu_cambio_obligatorio(num_jugador: int):

    equipo = pokemones_jugador(num_jugador)

    if num_jugador == 1:
        pokemon_activo = st.session_state.pokemon_activo_j1
    else:
        pokemon_activo = st.session_state.pokemon_activo_j2
    disponibles=[]
    for pokemon in equipo.values():
        if pokemon.hp>0 and pokemon.nombre != pokemon_activo.nombre:
            disponibles.append(pokemon)

    columnas = st.columns(len(disponibles))

    for i, pokemon in enumerate(disponibles):
        with columnas[i]:
            st.image(pokemon.sprite, width=120)
            st.markdown(f"{pokemon.nombre}")
            st.write(f" HP: {int(pokemon.hp)}")

            if st.button(
                f"Elegir",
                key=f"cambio_obligatorio_{num_jugador}_{pokemon.nombre}"
            ):
                ...
                st.success(f"¡{pokemon.nombre} entra al combate!")
                st.rerun()

def menu_combate():
    if "turno_resuelto" not in st.session_state:
        st.session_state.turno_resuelto = False
    if "log_turno" not in st.session_state:
        st.session_state.log_turno = []

    if st.session_state.pokemon_activo_j1.hp <= 0:
        if not equipo_tiene_vida(1):
            mostrar_pantalla_victoria(2)
            return
        st.warning(f"Jugador 1: tu {st.session_state.pokemon_activo_j1.nombre} se debilitó.")
        mostrar_menu_cambio_obligatorio(1)
        return

    if st.session_state.pokemon_activo_j2.hp <= 0:
        if not equipo_tiene_vida(2):
            mostrar_pantalla_victoria(1)
            return
        st.warning(f"Jugador 2: tu {st.session_state.pokemon_activo_j2.nombre} se debilitó.")
        mostrar_menu_cambio_obligatorio(2)
        return
    ...
    if st.session_state.accion_j1 is not None and st.session_state.accion_j2 is not None:
        ...
        if not st.session_state.turno_resuelto:
            log = []

            if accion1["tipo"] == "cambio":
                st.session_state.pokemon_activo_j1 = accion1["valor"]
                log.append(("info", f"¡El Jugador 1 cambió a {st.session_state.pokemon_activo_j1.nombre}!"))

            if accion2["tipo"] == "cambio":
                st.session_state.pokemon_activo_j2 = accion2["valor"]
                log.append(("info", f"¡El Jugador 2 cambió a {st.session_state.pokemon_activo_j2.nombre}!"))
            ...
            if accion1["tipo"] == "ataque" and accion2["tipo"] == "ataque":
                if pokemon1.velocidad > pokemon2.velocidad:
                    atacar(pokemon1, pokemon2, accion1["valor"], log)
                    if pokemon2.hp > 0:
                        atacar(pokemon2, pokemon1, accion2["valor"], log)
            ...
            st.session_state.log_turno = log
            st.session_state.turno_resuelto = True

        # --- ESTO SE MUESTRA SIEMPRE, sin importar cuántos reruns pasen ---
        mostrar_log(st.session_state.log_turno)
    ...
    if st.session_state.pantalla_actual == "menu":
        ...
        with col1:
            if st.button("JUGAR 1VS1"):
                ...
                st.rerun()
                st.audio("elegir_tema.mp3")
                    
        with col2:
            if st.button("JUGAR CONTRA BOT"):
                st.audio("elegir_tema.mp3", autoplay=True)
